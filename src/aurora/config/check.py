...
def validate_config(env: Mapping[str, str] | None = None) -> list[str]:
    ...
    if provider not in VALID_PROVIDERS:
        ...
        if key_env and not _value(env, key_env):
            problems.append(
                f"PROVIDER={provider} requires {key_env} in .env "
    ...
    if pin:
        from aurora.core.knowledge import KNOWLEDGE_ROOT, _snapshot_dirs
...
def require_valid_config(env: Mapping[str, str] | None = None) -> None:
    ...
    problems += check_env_file_permissions(Path.cwd() / ".env")
    if not problems:
        return
    print("Configuration problems (fix .env):")
